...
class WalletTransactions(models.Model):
    ...
    tranaction_type = models.CharField(max_length=10, choices=[('CREDIT', 'Received + '), ('DEBIT', 'Withdrawn - ')])
    ...
    def _update_account_balance(self, *args, **kwargs):
        if self.tranaction_type == "CREDIT":
            print("CREDIT")
            self.wallet_key.balance+=self.amount
        elif self.tranaction_type == "DEBIT":
            print("DEBIT")
    ...
    def __str__(self):
        return f"{self.amount} as {self.tranaction_type} for {self.wallet_key.user_code}"
