    ...
    def get(self, request):
        ...
        late_submissions = (
            ...
            + AssignedTask.objects.filter(
                student__student_code=user_data["user_code"], is_late_submission=True
            ).count()
        )
        ...
        on_time_rate = (
            (on_time_submissions / total_completed) * 100 if total_completed > 0 else 0
        )
...
class StudentLeaderBoardAndTableView(views.APIView):
    ...
    def get(self, request):
        ...
        student_sessions = StudentsInSession.objects.filter(
            session__session_code=session_code
        )
        ...
        assessment_scores = (
            ...
            .values("student_session__student_code")
            .annotate(total_assessment_score=Sum("score"))
        )

        task_scores = (
            AssignedTask.objects.filter(student__student_code__in=student_codes)
            .values("student__student_code")
            .annotate(total_task_score=Sum("score"))
        )
        ...
        recent_assessments = (
            ...
            .order_by("-started_on")[:3]
            .values("id", "assessment__assessment_title", "score")
        )
        ...
        recent_tasks = (
            ...
            .order_by("-completed_on")[:3]
            .values("task__title", "score")
        )
